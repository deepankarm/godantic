...
func main() {
	...
	payment, errs := validator.Unmarshal([]byte(creditCardJSON))
	if errs != nil {
	...
	payment, errs = validator.Unmarshal([]byte(paypalJSON))
	if errs != nil {
		fmt.Printf("Validation failed: %v\n", errs)
	} else {
	...
	_, errs = validator.Unmarshal([]byte(invalidJSON))
	if errs != nil {
	...
	for _, paymentJSON := range payments {
		payment, errs := validator.Unmarshal([]byte(paymentJSON))
		if errs != nil {
			continue
		}
